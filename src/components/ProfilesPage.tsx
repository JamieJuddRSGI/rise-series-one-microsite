...
export const ProfilesPage: React.FC<ProfilesPageProps> = ({ onNavigate }) => {
  ...
  const [selectedPractice, setSelectedPractice] = React.useState<string>('all');
  const [selectedLocation, setSelectedLocation] = React.useState<string>('all');

  const practiceOptions = ['all', ...PRACTICE_AREAS];
  const locations = ['all', ...Array.from(new Set(lawyers.map((l) => l.location))).sort()];

  const rankByTotalScore = React.useMemo(() => {
    if (selectedPractice === 'all') {
      return calculateRankingsWithTies(lawyers, (l) => l.totalScore).rankings;
    }
    const practiceAreaLawyers = lawyers.filter((l) =>
      getLawyerPracticeAreas(l).includes(selectedPractice),
    );
    return calculateRankingsWithTies(
      practiceAreaLawyers,
      (l) => getLawyerRanking(l, selectedPractice)?.totalScore ?? 0,
    ).rankings;
  }, [selectedPractice]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredLawyers.map((lawyer) => {
            ...
            const practiceAreaForScores = selectedPractice === 'all' ? null : selectedPractice;
            const scores = getScoreForPracticeArea(lawyer, practiceAreaForScores);
                ...
                <div className="p-6 flex-1 flex flex-col">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      ...
                      <p className="text-slate-500 text-sm">{lawyerPracticeAreas.join(', ')}</p>
                    </div>
                    ...
                    <div className="text-center">
                      <div className="h-1 bg-slate-300 rounded mb-1 overflow-hidden">
                        <div
                          ...
                          style={{ width: `${(scores.reputationScore / 10) * 100}%` }}
                        />
                      </div>
                    ...
                    <div className="text-center">
                      <div className="h-1 bg-slate-300 rounded mb-1 overflow-hidden">
                        <div
                          ...
                          style={{ width: `${(scores.instructionScore / 10) * 100}%` }}
                        />
                      </div>
